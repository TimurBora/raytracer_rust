...
impl<T: Copy + From<f64>, const N: usize> RaytracerVector<T, N> {
    ...
    pub const fn new_with_data(data: [T; N]) -> Self {
        Self { data }
    }
    ...
    fn apply_op_vector<F>(self, rhs: Self, op: F) -> Self
    where
        F: Fn(T, T) -> T,
    {
...
impl<T, const N: usize> Mul<Self> for RaytracerVector<T, N>
where
    ...
    T: Div<Output = T> + Into<f64> + From<f64> + Copy,
{
    ...
    pub fn normalize(&self, scale: Option<f64>) -> Self {
        let length = scale.map_or_else(|| self.length(), |scale| self.length() / scale);
    ...
    T: Mul<Output = T> + Sub<Output = T> + From<f64> + Copy,
{
    ...
    pub fn cross(self, vector: &Self) -> Self {
        ...
        Self::new_with_data([cx, cy, cz])
    }

    pub fn cross_static(vector1: &Self, vector2: &Self) -> Self {
